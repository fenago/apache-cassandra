...
#### You are writing a `SELECT` statement to query the `killrvideo.user_credentials` table. Since the primary key is the `email`, you want to search for the email value of `'cv@datastax.com'`.
...
#### You are writing an `UPDATE` statement for the `killrvideo.user_credentials` table. Since the primary key is the `email`, you want to update the row with the `email` value of `'cv@datastax.com'`. You will use the LWT with the form `IF password = '3@$tC0@$tC@ss@ndr@'`.
...
#### Retrieve the row from `killrvideo.user_credentails` where the `email` is `cv@datastax.com`.
...
#### **Step 1:** In the following cell, delete the row with the `email` of `cv@datastax.com` from the `user_credentials` table.
...
#### You will use the `DELETE` command on the `killrvideo.user_credentials` table. Since we want to delete all columns in the row, leave the column specification clause empty. In the `WHERE` clause, be sure to specify the `email` as `cv@datastax.com`


*Just want the command? Click here.*

    DELETE FROM killrvideo.user_credentials WHERE email = 'cv@datastax.com';
...
#### At this point you understand some of the advanced concepts of Cassandra. Want to learn more? Visit [DataStax Academy](https://academy.datastax.com/). But you need to hurry before we double the price (of course that means it's still free).
